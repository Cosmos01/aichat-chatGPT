...
class ChatGPT:
    ...
    def change_conversation(self,conversation_id) -> None:
        url = 'https://chat.openai.com/chat/' + conversation_id
        self.driver.get(url)
        self.__check_capacity(url)
        self.__check_blocking_elements()


    def select_last_conversation(self) -> None:
        self.reset_conversation()
        WebDriverWait(self.driver, 5).until(
            EC.element_to_be_clickable(chatgpt_chats_list_first_node)
        ).click()
    ...
    def try_again(self) -> str:
        ...
        self.driver.find_element(By.CSS_SELECTOR, '.btn.flex.justify-center.gap-2.btn-neutral.border-0').click()
